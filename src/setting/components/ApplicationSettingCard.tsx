...
export class ApplicationSettingCard extends Component<ApplicationSettingCardProps, ApplicationSettingCardState> {
    ...
    private enableFilterNonExistsFiles: boolean

    constructor(props: ApplicationSettingCardProps) {
        super(props)

        this.enableFilterNonExistsFiles = utools.dbStorage.getItem(enableFilterNonExistsFilesId(this.nativeId)) ?? false
    }

    switch(id: string, value: boolean) {
        utools.dbStorage.setItem(id, value)
        this.enableFilterNonExistsFiles = value
        this.update()
    }
